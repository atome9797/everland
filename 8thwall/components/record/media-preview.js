...
import {SaveMode, downloadResultBar} from '../../components/event/camera-components'
...
const share = async () => {
  ...
  if (navigator.canShare && navigator.canShare({files: [fileToInclude]})) {
    const shareObject = {
      title: '',
      text: '',
      files: [fileToInclude],
...
const showVideoPreview = ({videoBlob}) => {
  ...
  // videoPreview.addEventListener('loadeddata', stopRecMode)
  videoPreview.load()
}
...
const initMediaPreview = (options = {}) => {
  ...
  if ((navigator.canShare && navigator.canShare(shareTestObj) && window.XR8.XrDevice.deviceEstimate().os === 'iOS')) {
    document.getElementById('downloadButton').addEventListener('click', share)
  } else {

...
import {recMode, stopRecMode} from '../../components/event/camera-components'
...
const takeScreenshot = () => {
  $('.wrapMode').eq('1').css('display', 'none')
...
const initRecordButton = () => {
  ...
  const videoButton = document.querySelector('#recorder-button')
  const captureButton = document.querySelectorAll('.picture_shoot_btn')
  videoButton.addEventListener('touchstart', down)
  videoButton.addEventListener('mousedown', down)
  // captureButton.addEventListener('touchstart', down)
  // captureButton.addEventListener('mousedown', down)
  captureButton.forEach(target => target.addEventListener('touchstart', down))
  captureButton.forEach(target => target.addEventListener('mousedown', down))
